...
router.patch(
  ...
  asyncHandler(async (req, res) => {
    ...
    const { rows } = await pool.query(
      `SELECT id, client_user_id, is_disabled FROM job_posts WHERE id = $1`,
      [jobPostId.data],
    );
    if (!rows.length) throw new HttpError(404, 'Job post not found.');
    if (rows[0].client_user_id !== req.auth.sub) throw new HttpError(403, 'Only the owner can toggle this post.');

    const newDisabled = !rows[0].is_disabled;
    await pool.query(
      `UPDATE job_posts SET is_disabled = $1, updated_at = NOW() WHERE id = $2`,
      [newDisabled, jobPostId.data],
    );
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (original.client_user_id !== req.auth.sub) throw new HttpError(403, 'Only the owner can repost this job.');

    const { rows: existing } = await pool.query(
      `SELECT id FROM job_posts WHERE reposted_job_id = $1`,
      [jobPostId.data],
    );
    if (existing.length) throw new HttpError(409, 'This job has already been reposted once.');

    const { rows: created } = await pool.query(
      `INSERT INTO job_posts
        (client_user_id, post_type, title, category, municipality, location_details, description,
         budget_min, budget_max, workers_needed, allow_direct_booking, scheduled_at, reposted_job_id)
      ...
      [
        ...
        original.allow_direct_booking, original.scheduled_at, jobPostId.data,
      ],
    );
    res.status(201).json({ jobPost: created[0] });
  }),
);
